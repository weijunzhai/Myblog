	...
	class adminController{
		...
		public function __construct(){
			...
			if(!(isset($_SESSION['auth']))&&(PC::$method!='login')){
				$this->showmessage('Login Please！', 'admin.php?controller=admin&method=login');
			}else{
		...
		public function logout(){
			unset($_SESSION['auth']);
				$this->showmessage('Log Out Success！', 'admin.php?controller=admin&method=login');
		}
		...
		public function newsdel(){
			if($_GET['id']){
				$this->delnews();
				$this->showmessage('Blog Deleted！', 'admin.php?controller=admin&method=newslist');
			}
		}

		private function checklogin(){
			if(empty($_POST['username'])||empty($_POST['password'])){
				$this->showmessage('Login Failed！', 'admin.php?controller=admin&method=login');
			}
			...
			if($auth = $authobj->checkauth($username, $password)){
				$_SESSION['auth'] = $auth;
				$this->showmessage('Login Sucessed！', 'admin.php?controller=admin&method=newslist');
			}else{
				$this->showmessage('Login Failed！', 'admin.php?controller=admin&method=login');
			}
		}
		...
		private function newssubmit(){
			...
			if(empty($title)||empty($content)){
				$this->showmessage('Please submit after complete everything！', 'admin.php?controller=admin&method=newsadd');
			}
			...
			if($_POST['id']!=''){
				...
				$this->showmessage('Modify Successed！', 'admin.php?controller=admin&method=newslist');
			}else{
				$newsobj ->insert($data);
				$this->showmessage('Blog Added！', 'admin.php?controller=admin&method=newslist');
			}
		}
